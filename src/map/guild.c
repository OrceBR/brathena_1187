...
// ƒMƒ‹ƒh‚ÌEXPƒLƒƒƒbƒVƒ…‚Ìƒtƒ‰ƒbƒVƒ…‚ÉŠÖ˜A‚·‚é’è”
#define GUILD_SEND_XY_INVERVAL	5000	// À•W‚â‚g‚o‘—M‚ÌŠÔŠu
#define GUILD_PAYEXP_INVERVAL 10000	// ŠÔŠu(ƒLƒƒƒbƒVƒ…‚ÌÅ‘å¶‘¶ŠÔAƒ~ƒŠ•b)
#define GUILD_PAYEXP_LIST 8192	// ƒLƒƒƒbƒVƒ…‚ÌÅ‘å”
...
int guild_created(int account_id,int guild_id)
{
	...
	if(sd==NULL)
		return 0;
	if(!guild_id) {
		clif_guild_created(sd,2);	// ì¬¸”si“¯–¼ƒMƒ‹ƒh‘¶İj
		return 0;
	}
	//struct guild *g;
	...
	if(battle_config.guild_emperium_check)
		pc_delitem(sd,pc_search_inventory(sd,714),1,0,0);	// ƒGƒ“ƒyƒŠƒEƒ€Á–Õ
	return 0;
}
...
int guild_invite(struct map_session_data *sd,struct map_session_data *tsd)
{
	...
	if(!battle_config.invite_request_check) {
		if (tsd->party_invite>0 || tsd->trade_partner || tsd->adopt_invite ) {	// ‘Šè‚ªæˆø’†‚©‚Ç‚¤‚©
...
{ // cleaned up [LuzZza]
	...
	if(idx == -1 || c == 0) {
		// ƒMƒ‹ƒh‚Ìƒƒ“ƒo[ŠO‚È‚Ì‚Å’Ç•úˆµ‚¢‚·‚é
		struct map_session_data *sd = map_id2sd(account_id);
		if(sd && sd->status.char_id == char_id) {
			sd->status.guild_id=0;
			sd->guild_emblem_id=0;
...
int guild_reqalliance(struct map_session_data *sd,struct map_session_data *tsd)
{
	...
	if(agit_flag || agit2_flag)	{	// Disable alliance creation during woe [Valaris]
		clif_displaymessage(sd->fd,"Alianças não podem ser feitas durante guerras de Guildas!");
		return 0;
...
int guild_delalliance(struct map_session_data *sd,int guild_id,int flag)
{
	...
	if(agit_flag || agit2_flag)	{	// Disable alliance breaking during woe [Valaris]
		clif_displaymessage(sd->fd,"Alianças não podem ser quebradas durante guerras de Guildas!");
		return 0;
	}	// end addition [Valaris]
...
int guild_opposition(struct map_session_data *sd,struct map_session_data *tsd)
{
	...
	for(i=0;i<MAX_GUILDALLIANCE;i++){	// ‚·‚Å‚ÉŠÖŒW‚ğ‚Á‚Ä‚¢‚é‚©Šm”F
		if(g->alliance[i].guild_id==tsd->status.guild_id){
...
int guild_gm_changed(int guild_id, int account_id, int char_id)
{
	...
	{
		clif_displaymessage(g->member[pos].sd->fd, "Você não é mais o Mestre da Guilda.");
		g->member[pos].sd->state.gmaster_flag = 0;
	}

	if (g->member[0].sd && g->member[0].sd->fd)
	{
		clif_displaymessage(g->member[0].sd->fd, "Você agora é Mestre da Guilda!");
...
int guild_castlealldataload(int len,struct guild_castle *gc)
{
	...
	for( i = 0; i < n; i++, gc++ )
	{
		...
		if (!c) {
			ShowError("guild_castlealldataload Castelo id=%d nao encontrado.\n", gc->castle_id);
			continue;
		}
