...
bool pc_authok(struct map_session_data *sd, int login_id2, time_t expiration_time, int gmlevel, struct mmo_charstatus *st)
{
	...
	if (battle_config.display_version == 1){
		char buf[256];
		sprintf(buf, "brAthena Vers„o: %s", get_svn_revision());
...
{
	if (battle_config.use_statpoint_table || battle_config.use_statpoint2_table) //Use values from "db/statpoint.txt"
